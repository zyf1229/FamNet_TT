...
def extract_features(feature_model, image):
    
    Image_features = feature_model(image)

    return Image_features

def fuse_features(Image_features, image, boxes, feat_map_keys=['map3','map4'], exemplar_scales=[0.9, 1.1]):
    ...
    for ix in range(0,N):
        ...
        for keys in feat_map_keys:
            ...
            fusion_network = build_featurefusion_network(256,0.1,8,2048,4).to('cuda')    
            for j in range(0,M):
            ...
            #combined = fusion_network(examples_features,images_features)
